...
##
5.13
--OMG!昨天写的日志忘记COMMIT了。......WHAT CAN I SAY.
--
昨天看完redis基础，五种数据类型,增加，删除，查询操作，升降序。发布订阅模式-消息队列Stream-位图位域-事务-持久化-主从复制-哨兵模式。
基础操作都不难。ubantu中安装的版本太低了用不了一些操作，后面用的docker装新的。下午看面试题，为什么快？缓存，击穿，雪崩？持久化方式，区别？
为什么快？哨兵和集群？看累了之后，还看了堡垒机的基本操作，jumpserver。知识图谱昨天晚上太困了只做了一部分，今天上午在复习数据结构，复杂度-结构体-线性表中的顺序，链式存储
-栈后进先出，队列先进先出-二叉树，完全二叉树，满二叉树，三种遍历方式，节点，度就是节点分叉数量，叶节点-图，四种存储结构，邻接矩阵，邻接表，十字链表，邻接多重表，DFS一条路走到底，死路了再回头
BFS像水波扩散，先访问所有邻居，再一层层向外扩散。下午看了python的一些内容，基础语法，关键字，基本数据类型，感觉元组tuple，列表list，字典dictionary,长得挺像，基本语法操作也差不多，我也晓得是还没深入。
之后用claude指导利用github action，完成自动构建，推送Docker镜像的流水线。说来就有的说了，可能他给我的用户形象设置的很聪明，步骤有些过于少了，最开始是NAME写错写成电话号码，secret中又没写东西->Username and password requre，后面改，response from daemon 进入不了网址，总在Login to Aliyun Registry 里登录失败。开始就感觉给我写的yml文件中网址是不是有问题，它说又名字的问题，改为了用户名。依然报错，我还用了命名空间里的名字
，无用，最后交给gemini，看出地址少了一部分，终于解决，累死了，感觉遇到的好多问题都是因为这种小细节导致的。但我还是觉得起码sonnet4.6是有点问题的，起码对我来说，明天总复习，必须面对自己了。

--
##
5.14
上午还搞了Jenkins的一些操作，其实也就是类似push的操作。k8s基础操作解决，所用时间来说比想象中更久些，也有可能是因为今天学习时间断断续续，最后搞到七点，好在知识图谱做完了。
晚上搞简历感到心累，想要去除那个水印。听很多人推荐再overleaf里用LaTeX,还有再word里搞的，但感觉是不是又更简单的方法。gpt生成的把脸p没了。
心里总还想着数据结构的考试，总复习还没开始，周末一定搞完。
